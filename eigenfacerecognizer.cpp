	...
	bool EigenfaceRecognizer::loadTrainingImages()
	{
		...
		for (boost::filesystem::directory_iterator file(dir); file != end; ++file)
		{
			if (is_regular_file(*file))
				this->addTrainingImage(file->path().string(), file->path().filename().string());
		}

		if (trainingImages.size() < 2)
			return false;

		if (images.size() > 0 && numTrainedImages > 0)
		...
		{
			images[currentImage] = cv::imread(current->first.c_str(), CV_LOAD_IMAGE_GRAYSCALE);

			if (images[currentImage].empty()) break;
